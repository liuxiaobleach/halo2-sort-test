...
impl<F: FieldExt> RangeCheckChip<F> {
    ...
    fn assign_data(&self, mut layouter: impl Layouter<F>, val: Value<F>) -> Result<AssignedCell<F, F>, Error> {
        let config = self.config();

        layouter.assign_region(|| "load private data", |mut region| {
...
impl<F: FieldExt> Circuit<F> for RangeCheckCircuit<F> {
    ...
    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<F>) -> Result<(), Error> {
        ...
        let assigned_cell = range_check_chip.assign_data(layouter.namespace(|| "assign all"), Value::known(self.val))?;
...
fn main() {
    ...
    let a = Fp::from(1024 * 64 - 1);
    let b = Fp::from(1024 * 64);
    let ins = Fp::from(9);
